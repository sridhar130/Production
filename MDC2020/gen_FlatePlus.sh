#!/usr/bin/bash
#
#  First argument is production version
#
...
rm template.fcl
echo '#include "Production/JobConfig/primary/FlatePlus.fcl"' >> template.fcl
...
#
generate_fcl --dsconf=MDC2020$1 --dsowner=brownd --run-number=1210 --description=FlatePlus --events-per-job=4000 --njobs=100 \
--embed template.fcl --auxinput=1:physics.filters.TargetStopResampler.fileNames:MuminusStopsCat.txt
for dirname in 000 001 002 003 004 005 006 007 008 009; do
 if test -d $dirname; then
  echo "found dir $dirname"
  rm -rf FlatePlus$1_$dirname
  mv $dirname FlatePlus$1_$dirname
 fi
done
