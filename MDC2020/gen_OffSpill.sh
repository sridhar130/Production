...
# $3 is the campaign version (input primary and output digi)
# $4 is the number of input collections to merge (merge factor)
rm template.fcl
...
generate_fcl --dsconf="$2$3" --dsowner=mu2e --description="$1OffSpill" --embed template.fcl \
  --inputs="$1$3.txt" --merge-factor=$4
