...
then
  ...
  samListLocations ${SAMOPT} --defname="dig.${OWNER}.${PRIMARY}${DIGITYPE}${STREAM}.${CAMPAIGN}${DIGI_VERSION}_${DB_PURPOSE}_${DB_VERSION}.art" > Digis.txt
fi
...
for dirname in 000 001 002 003 004 005 006 007 008 009; do
  if test -d $dirname; then
    ...
    MDIR="${PRIMARY}${DIGITYPE}${STREAM}Reco_${dirname}"
    if test -d $MDIR; then
      echo "removing $MDIR"
      rm -rf $MDIR
    fi
