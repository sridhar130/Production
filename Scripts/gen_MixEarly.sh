#!/usr/bin/bash
#
# version of gen_Mix that uses the early (prescaled but no time constant) pileup collections
...
samweb list-file-locations --schema=root --defname="dts.mu2e.EarlyMuBeamFlashCat.$2$3.art"  | cut -f1 > MuBeamFlashCat$3.txt
samweb list-file-locations --schema=root --defname="dts.mu2e.EarlyEleBeamFlashCat.$2$3.art"  | cut -f1 > EleBeamFlashCat$3.txt
samweb list-file-locations --schema=root --defname="dts.mu2e.EarlyNeutralsFlashCat.$2$3.art"  | cut -f1 > NeutralsFlashCat$3.txt
...
nfiles=`samCountFiles.sh "dts.mu2e.EarlyMuBeamFlashCat.$2$3.art"`
nevts=`samCountEvents.sh "dts.mu2e.EarlyMuBeamFlashCat.$2$3.art"`
let nskip_MuBeamFlash=nevts/nfiles
nfiles=`samCountFiles.sh "dts.mu2e.EarlyEleBeamFlashCat.$2$3.art"`
nevts=`samCountEvents.sh "dts.mu2e.EarlyEleBeamFlashCat.$2$3.art"`
let nskip_EleBeamFlash=nevts/nfiles
nfiles=`samCountFiles.sh "dts.mu2e.EarlyNeutralsFlashCat.$2$3.art"`
nevts=`samCountEvents.sh "dts.mu2e.EarlyNeutralsFlashCat.$2$3.art"`
...
echo '#include "Production/JobConfig/mixing/EarlyMixins.fcl"' >> template.fcl
#
# set the skips
#
...
for dirname in 000 001 002 003 004 005 006 007 008 009; do
  if test -d $dirname; then
    ...
    rm -rf "$1MixEarly_$dirname"
    mv $dirname "$1MixEarly_$dirname"
  fi
done
