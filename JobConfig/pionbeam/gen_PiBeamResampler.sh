...
dataset=sim.mu2e.PiBeamCat.$1$2.art
samweb list-file-locations --schema=root --defname="$dataset"  | cut -f1 > PiBeamCat.txt
...
#
generate_fcl --dsconf=$1$3 --dsowner=sophie --run-number=1202 --description=PiBeamResampler --events-per-job=400000 --njobs=$4 \
  --embed pibeamresampler.fcl --auxinput=1:physics.filters.beamResampler.fileNames:PiBeamCat.txt
for dirname in 000 001 002 003 004 005 006 007 008 009; do
 if test -d $dirname; then
  echo "found dir $dirname"
  rm -rf PiBeamResampler_$dirname
  mv $dirname PiBeamResampler_$dirname
 fi
done
